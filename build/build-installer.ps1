...
& $iscc "/DMyAppVersion=$ver" (Join-Path $PSScriptRoot "SonarArchivo.iss")
if ($LASTEXITCODE -eq 0) {
    Write-Host "`nInstalador: $(Join-Path $root ("installer\SonarArchivo-Setup-$ver.exe"))" -ForegroundColor Green
} else {
